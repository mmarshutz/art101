...
var NodesJs = function (parameters) {
  ...
  t_NodesJs.particleSize = parameters.particleSize
    ? parameters.particleSize
    : 2;
  t_NodesJs.lineSize = parameters.lineSize ? parameters.lineSize : 1;
  t_NodesJs.particleColor = parameters.particleColor
    ? "rgba(" + parameters.particleColor.join(",") + ")"
    : "rgba(255,255,255,0.3)";
  t_NodesJs.lineColor = parameters.lineColor
  ...
  t_NodesJs.pointerCircleRadius = parameters.pointerCircleRadius
    ...
    function () {
      ...
      var step = function () {
        ...
        if (!t_NodesJs.nobg) {
          var r = Math.floor(
            ((Math.sin(
              (Math.PI * 2 * Date.now()) / t_NodesJs.backgroundDuration -
                Math.PI / 2
            ) +
              1) /
              2) *
              (t_NodesJs.backgroundFrom[0] - t_NodesJs.backgroundTo[0] + 1) +
              t_NodesJs.backgroundTo[0]
          );
          var g = Math.floor(
            ((Math.sin(
              (Math.PI * 2 * Date.now()) / t_NodesJs.backgroundDuration -
              ...
              1) /
              2) *
              (t_NodesJs.backgroundFrom[1] - t_NodesJs.backgroundTo[1] + 1) +
              t_NodesJs.backgroundTo[1]
          );
          var b = Math.floor(
            ((Math.sin(
              (Math.PI * 2 * Date.now()) / t_NodesJs.backgroundDuration -
              ...
              1) /
              2) *
              (t_NodesJs.backgroundFrom[2] - t_NodesJs.backgroundTo[2] + 1) +
              t_NodesJs.backgroundTo[2]
          );
